...
// 核心固定字段（不参与自定义管理）
const SYSTEM_FIELDS = ['id', 'studentId', 'name', 'createdAt', 'updatedAt'];
...
const FIELD_GROUPS = {
  ...
  '家庭信息': ['parentName', 'parentPhone', 'emergencyContact', 'emergencyPhone', 'nativePlace', 'highSchool'],
  '生涯发展': ['careerGoal'],
  '其他信息': []
};
...
function renderCellValue(fieldKey, value) {
  ...
  if (fieldKey === 'name') return desensitizeName(value);
  if (fieldKey === 'careerGoal') return `<span class="tag tag-blue">${value}</span>`;
  if (fieldKey === 'phone') return desensitizePhone(value);
...
async function showStudentDetail(id) {
  ...
  showModal(`${desensitizeName(student.name)} - 学生详情`, `
...
function renderStudentInfoGroups(student) {
  ...
  Object.entries(groups).forEach(([groupName, keys]) => {
    ...
    keys.forEach(key => {
      ...
      // 隐私脱敏
      if (key === 'name') value = desensitizeName(value);
      else if (key === 'phone') value = desensitizePhone(value);
      else if (key === 'address') value = desensitizeAddr(value);
      else if (key === 'parentPhone' || key === 'emergencyPhone' || key === 'idCard') {
        value = desensitizePhone(value);
      }
...
function showFieldManager() {
  ...
  // 确保核心字段存在且不可隐藏/删除
  const coreKeys = ['studentId', 'name'];

  const bodyHTML = `
    <div style="margin-bottom:16px">
      <div style="font-size:13px;color:var(--text-muted);margin-bottom:12px">
        勾选「显示在列表」的字段会出现在学生总览表中。您可以通过上移/下移调整列表中的显示顺序。
      </div>
  ...
  return `
    <tr data-key="${field.key}">
      ...
      <td style="text-align:center">
        <input type="checkbox" class="field-show" data-key="${field.key}" ${isInList ? 'checked' : ''} ${isCore ? 'checked disabled' : ''}>
      </td>
      <td style="text-align:center">
...
async function saveFieldManager() {
  ...
  // 读取列表显示设置
  state.studentListColumns = ['studentId', 'name'];
...
function updateGradeRanking() {
  ...
  document.getElementById('gradeRankBody').innerHTML = ranked.map((r, i) => `
    <tr class="row-clickable" onclick="showStudentDetail(${r.student.id})">
      ...
      <td>${desensitizeName(r.student.name)}</td>
...
async function updateGradeCompare() {
  ...
  body.innerHTML = gradeRanked.map((g, i) => {
    ...
    return `<tr>
      ...
      <td>${desensitizeName(g.student.name)}</td>
...
async function updateAssessCompare() {
  ...
  body.innerHTML = assessRanked.map((a, i) => {
    ...
    return `<tr>
      ...
      <td>${desensitizeName(a.student.name)}</td>
